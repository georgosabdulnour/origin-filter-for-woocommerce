...
function ofwc_filter_orders_by_origin()
{
    ...
    if ('shop_order' === $typenow) {
        ...
        $cache_key = 'ofwc_filter_origins';
        $cache_group = 'ofwc_origin_filter';
        ...
        if (false === $origins) {
            ...
            $origins = $wpdb->get_col(
                $wpdb->prepare(
                    ...
                    FROM %1$s.postmeta pm
                    INNER JOIN %1$s.posts p ON p.ID = pm.post_id
                    WHERE pm.meta_key = %2$s
                    AND p.post_type = %3$s
                    AND pm.meta_value != ''
                    ",
                    $wpdb->prefix,
        ...
        <select name="order_origin" id="order_origin">
            <option value=""><?php esc_html_e('All Origins', 'origin-filter-for-woocommerce'); ?></option>
            <?php
